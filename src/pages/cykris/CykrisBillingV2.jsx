...
export default function CykrisBillingV2() {
  ...
  const saveLatestWaybillToStorage = (waybill) => {
    ...
    const stored = localStorage.getItem('cykris_latestWaybill');
    const highest = compareWaybillNumbers(stored || '000-0000', waybill);
    localStorage.setItem('cykris_latestWaybill', highest);
  };
  ...
  useEffect(() => {
    if (billingStatement.length > 0) {
      ...
      if (currentStatementName !== 'Untitled') {
        // Save to named statement
        setSavedStatements(prevSavedStatements => {
          ...
          localStorage.setItem("savedCykrisBillingStatements", JSON.stringify(updatedSavedStatements));
      ...
      // Always save to main storage for auto-load on page reload (even if Untitled)
      localStorage.setItem("cykrisBillingStatement", JSON.stringify(saveData));
      // Also save current statement name if it's not Untitled
      if (currentStatementName !== 'Untitled') {
        localStorage.setItem("currentCykrisStatementName", currentStatementName);
      }
    }
  ...
  useEffect(() => {
    const backupInterval = setInterval(() => {
      if (billingStatement.length > 0 && currentStatementName !== 'Untitled') {
        ...
        localStorage.setItem("cykrisBillingStatement_backup", JSON.stringify(backupData));
  ...
  useEffect(() => {
    ...
    const currentStatementName = localStorage.getItem("currentCykrisStatementName");
    const savedStatements = localStorage.getItem("savedCykrisBillingStatements");
    ...
    if (currentStatementName && savedStatements) {
      try {
        ...
        localStorage.removeItem("currentCykrisStatementName");
        localStorage.removeItem("savedCykrisBillingStatements");
      }
    }

    // Fallback to old system if new system doesn't have data
    const saved = localStorage.getItem("cykrisBillingStatement");
    if (saved) {
      try {
        ...
        const backup = localStorage.getItem("cykrisBillingStatement_backup");
        if (backup) {
          try {
            ...
            localStorage.removeItem("cykrisBillingStatement");
            localStorage.removeItem("cykrisBillingStatement_backup");
          }
        } else {
  ...
  const addToBillingStatement = (dr) => {
    ...
    const newItem = {
      ...
      // Add quantity, unit, description, and type fields
  ...
  const loadSavedStatements = () => {
    try {
      const saved = localStorage.getItem("savedCykrisBillingStatements");
      if (saved) {
      ...
      {showBillingRecords && (
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            disabled={isSavingDr}
            className="hidden px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {isSavingDr ? (
              <>
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200 min-h-[400px]">
          <table className="w-full text-sm text-left text-gray-700 bg-white">
            ...
            <tbody className="billing-records-table">
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={3} className="text-center text-gray-400 py-8">Loading records...</td>
                </tr>
              ) : (
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                className="hidden px-4 py-2 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 border border-blue-200"
          ...
          <table className="w-full text-sm text-left text-gray-700 bg-white">
            <thead className="text-xs text-gray-700 bg-gray-100 sticky top-0 z-1">
              <tr>
                ...
                <th className="px-3 py-3">Declared Amount</th>
      ...
      */}
        ...
        <div style={{ display: "none" }}>
               ...
               for (let i = 0; i < billingStatement.length; i += itemsPerPage) {
                 ...
                 pages.push(
                   <div key={i} style={{ position: 'relative', pageBreakAfter: 'always', minHeight: '1200px', color: color }}>
                     ...
                     {/* Print Title */}
                     <div className="print-title" style={{ position: 'absolute', top: '221px', left: '210px' }}>CYKRIS ENTERPRISES</div>
                     
                     {/* Print Address */}
                     <div className="print-address" style={{ position: 'absolute', top: '297px', left: '210px' }}>P1 B1 L23 DECA HOMES, INDANGAN HIGHWAY, DAVAO CITY</div>

                     {/* Print Data Table */}
                     <table className="print-data-table" style={{ visibility: '', position: 'absolute', top: '366px', left: '0px' }}>
                     ...
                     <table className="print-data-table" style={{ visibility: 'hidden', position: 'absolute', top: '366px', left: '0px' }}>
                       <thead>
                         <tr>
          ...
          <div ref={printRef}>
            ...
            <div style={{ fontFamily: 'Arial', fontSize: '8px', fontWeight: 'bold' }}>P1 B1 L23 DECA HOMES, INDANGAN HIGHWAY, DAVAO CITY</div>
